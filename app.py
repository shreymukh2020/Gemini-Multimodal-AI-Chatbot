...
# Default model for text generation, fallback to gemini-1.5-flash
...
if selected == '🤖 AI ChatBot':
    gemini_pro_model = get_model("gemini-1.5-flash")  # Use gemini-1.5-flash as fallback model
    st.title("🤖 AI ChatBot")
    st.markdown("Chat with Gemini AI and get instant responses.", unsafe_allow_html=True)
    ...
    user_prompt = st.chat_input("Ask Gemini AI...")
    if user_prompt:
...
# Image Captioning Page
    ...
    if uploaded_image and st.button("Generate Caption"):
        ...
        with col2:
            gemini_vision_model = get_model("gemini-1.5-flash")  # Using gemini-1.5-flash here as well
...
# Text Generation Page
    ...
    st.markdown("Provide a prompt, and Gemini AI will generate text based on your input.", unsafe_allow_html=True)
    ...
    if st.button("Generate Text"):
        if user_prompt.strip():
            try:
                gemini_pro_model = get_model("gemini-1.5-flash")  # Using gemini-1.5-flash for text generation
...
# Text Summarization Page
    ...
    st.markdown("Provide a long piece of text, and Gemini AI will summarize it for you.", unsafe_allow_html=True)
    ...
    if st.button("Generate Summary"):
        if user_prompt.strip():
            try:
                gemini_pro_model = get_model("gemini-1.5-flash")  # Using gemini-1.5-flash for summarization
